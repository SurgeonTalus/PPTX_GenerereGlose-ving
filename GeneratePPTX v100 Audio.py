...
# Store the time of the last loop run (initialize it with the current time)
    ...
    IMGinstruct = " " + "Based on the given input text write out details for one illustration."
    ...
    def translate_prompt_text(prompt_text):
        ...
        url = "http://localhost:1234/v1/chat/completions"
        request_data = {
            "messages": [{"role": "system", "content": IMGincludeFilenameToForceTopic + IMGinstruct + IMGnoChatting + IMGlanguage + IMGEducatioanl + IMGautonomy + IMGpreferObjects + IMGifCharacters + IMGifHumans + IMGmood + IMGobjects + IMGconsise + IMGbacground + ""}],
...
################# Prompt Directly into DrawThings (dt) ################
        ...
        dtStoryTelling = "" + "Candid shot, cinematic, documentary"  + " " #For better scene composition with humans include:
        dtArnonDuglas = "" + "In the style of Aaron Douglas" + " " #Modern Dramatic Illustrator Cartoons, shadowy
